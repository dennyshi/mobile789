...
switch ($action) {
    case 'register'://会员注册
        ...
        $intr = filter_input(INPUT_POST, 'intr');//代理推广
        $sp = filter_input(INPUT_POST, 'sp');//会员推广
        $truename = $username;
        $passwd = $password;
        $agree = 1;
        if (
                ...
                empty($agree) ||
                $password != $passwd ||
                !CommonClass::check_username_str($username, 4, 12) ||
    ...
    case 'checpwd'://会员修改密码验证旧密码
        ...
        $ypassword = filter_input(INPUT_POST, 'ypassword');
...
//            $params = array('username' => 'aabb5566', 'company' => SITE_ID, 'userpass' => 'bbaa5566', 'ip' => $ip, 'user_type' => SITE_TYPE); //主副站修改
    ...
    case 'changepassword'://修改密码
        ...
        $user_type = filter_input(INPUT_POST, 'user_type'); //主副站修改
    ...
    case 'changegetpassword'://修改密码
        ...
        $user_type = filter_input(INPUT_POST, 'user_type'); //主副站修改
        $newpwd = CommonClass::get_md5_pwd($password);
        $params = array('username' => $username, 'newuserpass' => $newpwd, 'company' => SITE_ID, 'oid' => $oid, 'ip' => $ip, 'user_type' => $user_type); //主副站修改
    ...
    case 'savebankorder'://提交公司存款订单
        $ulevel = filter_input(INPUT_POST, 'ulevel');
        $params['username'] = strtolower(filter_input(INPUT_POST, 'username'));
        $params['site_id'] = SITE_ID;
        $params['ip'] = $ip;
        $params['bank_set_id'] = filter_input(INPUT_POST, 'in_bank_set_id'); //收款人配置id
        $params['bank_id'] = filter_input(INPUT_POST, 'out_bank_id');   //存款人银行id
        $params['card_user'] = filter_input(INPUT_POST, 'user');        //存款人真实姓名
        $params['pay_type'] = filter_input(INPUT_POST, 'type');         //支付类型
        $params['pay_ip'] = $ip;
        $params['money'] = filter_input(INPUT_POST, 'money');
        $params['billno'] = filter_input(INPUT_POST, 'billno');
        $params['oid'] = filter_input(INPUT_POST, 'oid');
        if (
                empty($params['billno']) ||
    ...
    case 'saveuserdetial'://绑定银行卡
        ...
        $params['username'] = strtolower(filter_input(INPUT_POST, 'username'));
        $params['oid'] = filter_input(INPUT_POST, 'oid');
        $params['bank_id'] = filter_input(INPUT_POST, 'bank_id');
        $params['bank_address'] = filter_input(INPUT_POST, 'bank_address');
        $params['bank_account'] = filter_input(INPUT_POST, 'bank_account');
        $params['bank_password'] = filter_input(INPUT_POST, 'bank_password');
        if (
                empty($params['bank_id']) ||
    ...
    case 'changeuserdetail'://更改会员基本资料
        ...
        $params['username'] = strtolower(filter_input(INPUT_POST, 'username'));
        $params['oid'] = filter_input(INPUT_POST, 'oid');
        $params['ty'] = filter_input(INPUT_POST, 'ty');
        $params['chgvalue'] = filter_input(INPUT_POST, 'chgvalue');
